...
bool contains(UniqueSet *set, u8 *data, u32 length) {
  UniqueValue *cur = set->head;
  while (cur) {
    if (cur->length == length && memcmp(cur->data, data, length) == 0) {
      return true;
    }
    cur = cur->next;
  }
...
// Helper: hex string 출력용
void print_hex(FILE *fp, unsigned char *data, unsigned int len) {
...
void insert(UniqueSet *set, u8 *data, u32 length) {
  if (set->contains(set, data, length)) return;

  UniqueValue *new_val = ck_alloc(sizeof(UniqueValue));
  new_val->data = ck_alloc(length);
  memcpy(new_val->data, data, length);
  new_val->length = length;
  new_val->next = set->head;
  set->head = new_val;
  set->count++;
}

void init_value_sets() {
  enum_value_set = ck_alloc(sizeof(UniqueSet));
  enum_value_set->head = NULL;
  enum_value_set->insert = insert;
  enum_value_set->contains = contains;
  enum_value_set->count = 0;

  length_value_set = ck_alloc(sizeof(UniqueSet));
  length_value_set->head = NULL;
  length_value_set->insert = insert;
  length_value_set->contains = contains;
  length_value_set->count = 0;

  offset_value_set = ck_alloc(sizeof(UniqueSet));
  offset_value_set->head = NULL;
  offset_value_set->insert = insert;
  offset_value_set->contains = contains;
  offset_value_set->count = 0;
}
...
Track *parse_constraint_file(u8 *path, u8 *in_buf) {
  ...
  track->enums = NULL;
  track->enum_number = 0;
  track->length_number = 0;
  track->offset_number = 0;
  for (u32 i = 0; i < num; i++) {
...
void reusing_stage(char **argv, u8 *buf, u32 len, Chunk *tree, Track *track){
  ...
  if (save_length_number > 512) {
    save_length_number = 512;
  }
  if (save_length_number == 0) {
    save_length_number = 100;
  }
  u32 length_threshold = track->length_number / save_length_number;
  if (length_threshold == 0) {
    length_threshold = 1;
  }

  save_offset_number = (save_offset_number + track->offset_number) / 2;
  if (save_offset_number > 512) {
    save_offset_number = 512;
  }
  if (save_offset_number == 0) {
    save_offset_number = 100;
  }
  u32 offset_threshold = track->offset_number / save_offset_number;
  ...
  // for(int i=0;i<max_iteration;i++){
    ...
    //   u32 new_offset = 0;
    //   if (meta_length == 1)
    //     new_offset = *(u8 *)cur->data;
    //   else if (meta_length == 2)
    //     new_offset = *(u16 *)cur->data;
    //   else if (meta_length == 4)
    //     new_offset = *(u32 *)cur->data;

    //   u8 *payload_buf = ck_alloc(payload_length);
    //   memcpy(payload_buf, out_buf + offset_iter->target_start, payload_length);
    //   out_buf = delete_data(out_buf, &out_len, offset_iter->target_start, payload_length);
    //   if (new_offset + payload_length <= out_len) {
    //     out_buf = copy_and_insert(out_buf, &out_len, new_offset, 0, 0); // 공간 확보
    //     memcpy(out_buf + new_offset, payload_buf, payload_length);
    //   } else {
    //     // 끝에 삽입 (부분만)
    //     u32 safe_len = (out_len > new_offset) ? (out_len - new_offset) : 0;
    //     if (safe_len > 0) {
    //       memcpy(out_buf + new_offset, payload_buf, safe_len);
    //     }
    //   }
    //   ck_free(payload_buf);
